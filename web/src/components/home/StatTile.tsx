...
export function StatTile({ label, value, change, spark, foot, explain, children }: { label: ReactNode; value: string; change?: number | null; spark?: number[]; foot?: string; explain?: { topic: ExplainTopic; live?: Live; tourId?: string }; children?: ReactNode }) {
  return (
    <Card className="glass-hover flex min-h-[132px] min-w-0 flex-col justify-between p-4 sm:p-5">
      ...
      <div className="mt-3 flex min-w-0 items-end justify-between gap-2">
        ...
        {children}
        {explain && <ExplainButton topic={explain.topic} live={explain.live} tourId={explain.tourId} className="-mb-1 -mr-1.5" />}
      </div>
    </Card>
  );
}
