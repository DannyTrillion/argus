  ...
  return (
    <div className="space-y-5">
      ...
      <motion.div {...fade} transition={{ duration: 0.4, delay: 0.05 }} data-tour="pulse" className="grid min-w-0 grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-5">
        {g ? (
          <>
            <StatTile label="Total market cap" value={usd(g.total_market_cap, { compact: true })} change={g.total_market_cap_yesterday_percentage_change} spark={spark("total_market_cap")} explain={{ topic: "market-cap", live: { value: g.total_market_cap, change: g.total_market_cap_yesterday_percentage_change } }} />
            <StatTile label="24h volume" value={usd(g.total_volume_24h, { compact: true })} change={g.total_volume_24h_yesterday_percentage_change} spark={spark("total_volume_24h")} explain={{ topic: "volume", live: { value: g.total_volume_24h, change: g.total_volume_24h_yesterday_percentage_change } }} />
            <StatTile label="BTC dominance" value={`${g.btc_dominance.toFixed(2)}%`} change={g.btc_dominance_24h_percentage_change} spark={spark("btc_dominance")} foot={`ETH ${g.eth_dominance.toFixed(1)}%`} explain={{ topic: "dominance", live: { value: g.btc_dominance, change: g.btc_dominance_24h_percentage_change } }} />
            <Card className="flex flex-col items-center justify-center gap-1 p-3 sm:p-4">
              <div className="flex w-full items-center justify-between gap-1 text-[12px] text-ink-2">
                ...
                <ExplainButton topic="fear-greed" live={{ value: fg?.value }} className="-mr-1.5" />
              </div>
            ...
            <Card className="col-span-2 flex flex-col items-center justify-center gap-1 p-3 sm:p-4 lg:col-span-1">
              <div className="flex w-full items-center justify-between gap-1 text-[12px] text-ink-2">
                ...
                <ExplainButton topic="altcoin-index" live={{ value: alt?.altcoin_index }} className="-mr-1.5" />
              </div>
